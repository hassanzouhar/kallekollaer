...
export const RetroLayout: React.FC<RetroLayoutProps> = ({ children, title, wallet }) => {
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-[#33ff00] font-mono relative selection:bg-[#33ff00] selection:text-black">
      ...
      {/* Main Container */}
      <div className="relative z-10 max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-6 min-h-screen flex flex-col">
        {/* Header */}
        <header className="border-b-2 border-green-800/50 pb-2 sm:pb-3 mb-3 sm:mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-end uppercase tracking-widest shrink-0 gap-2 sm:gap-0">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold text-glow font-vt323 leading-none tracking-wide">Retro Hockey Manager</h1>
            <p className="text-[10px] sm:text-xs opacity-60 tracking-[0.2em] mt-1">Norwegian U18 Elite Edition</p>
          </div>
        ...
        {/* Content Frame */}
        <main className="flex-1 border-x border-green-900/30 bg-[#050a05] p-2 sm:p-3 md:p-4 overflow-y-auto relative custom-scrollbar shadow-inner min-h-0">
           {title && (
             <div className="flex justify-between items-center mb-4 sm:mb-6 border-b border-green-900/50 pb-2 sticky top-0 bg-[#050a05] z-30 pt-2">
               <div className="bg-green-900/20 text-green-300 px-2 sm:px-3 py-1 inline-block font-bold text-sm sm:text-base md:text-lg border-l-4 border-green-600 uppercase">
                 {title}
               </div>
               {wallet !== undefined && (
                 <div className="md:hidden text-green-400 font-bold text-xs sm:text-sm">
                   {wallet} PØKKS
                 </div>
               )}
             </div>
